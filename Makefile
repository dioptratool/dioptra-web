...
TRANSACTION_STORE_DATABASE_NAME ?= dioptra_transactions
TRANSACTION_STORE_USER ?= dioptra
TRANSACTION_STORE_PASSWORD ?= $(DB_PASSWORD)
TRANSACTION_STORE_HOST ?= localhost
TRANSACTION_STORE_PORT ?= 9005
TRANSACTION_STORE_DB_URL ?= postgres://$(TRANSACTION_STORE_USER)@$(TRANSACTION_STORE_HOST):$(TRANSACTION_STORE_PORT)/$(TRANSACTION_STORE_DATABASE_NAME)
...
restore-transactions-db: env-BACKUP env-TRANSACTION_STORE_DB_URL env-TRANSACTION_STORE_PASSWORD
	@PGPASSWORD="$(TRANSACTION_STORE_PASSWORD)" psql -v ON_ERROR_STOP=1 -q "$(TRANSACTION_STORE_DB_URL)" -f "$(BACKUP)"
