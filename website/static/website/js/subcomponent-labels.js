$(() => {
    ...
    on_label_remove = (event) => {
        ...
        if (subcomponent_labels_json.length < maxlabels) {
            $('#add-label-link').show()
        }
        trigger_change($('#subcomponent-label-type-fields'), arraysEqual(subcomponent_labels_json, subcomponent_labels_old))
    }
    ...
    $('#add-label-input').on('keydown', (event) => {if (event.key == "Enter") {
        event.preventDefault()
        on_add_label_btn(event.target)
    }})
