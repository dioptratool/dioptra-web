...
function handleCKEditorChange($form) {
  $form.find('textarea.django_ckeditor_5').addClass('notrackchange');

  function attachCKEditor5Listeners() {
    if (!window.editors) {
      return;
    }

    $form.find('textarea.django_ckeditor_5').each(function() {
      var textarea = this;
      var editor = window.editors[textarea.id];
      if (editor && !textarea._ck5ChangeAttached) {
        textarea._ck5ChangeAttached = true;
        var initialData = editor.getData();
        editor.model.document.on('change:data', function() {
          var $formGroup = $(textarea).parents('.form-group');
          var isDirty = editor.getData() !== initialData;
          $formGroup
            .toggleClass('changed', isDirty)
